...
class MMSolver(nn.Module):
    ...
    def __init__(self, geometry, dt: float, batch_size: int, sources=[], probes=[]):
        ...
        m0[
            :,
            1,
        ] = 1  # set initial magnetization in y direction
        self.m_history = []
    ...
    def run_stage(self, m, B_ext, Msat, signal):
        ...
        for sig in signal.split(1, dim=1):
            ...
            outputs = checkpoint(
                self.measure_probes, m, Msat, outputs, use_reentrant=False
            )
    ...
    def torque_LLG(self, m, B_ext, Msat, relax=False):
        ...
        return -(
            ...
            * (
                m_x_Beff + self.Alpha(relax) * cross(m, m_x_Beff, 1)
            )  # this is going to infinity which is causing everything to go to nan
