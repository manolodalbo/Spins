...
class WaveGeometryFreeForm(WaveGeometry):
    def __init__(self, dim: tuple, d: tuple, B0: float, B1: float, Ms: float):
        ...
        self.rho = nn.Parameter(torch.normal(mean=0, std=0, size=dim))
...
class WaveGeometryArray(WaveGeometry):
    def __init__(
        ...
        self.convolver = nn.Conv2d(
            3, 3, self.dm, padding=(self.dm // 2), groups=3, bias=False
        )
        self.convolver.weight.requires_grad = False

        for i in range(3):
            self.convolver.weight[
                i,
                0,
            ] = ones((dm, dm))
    ...
    def forward(self):
        ...
        B_demag = real(
            ifftn(
                torch.stack(
                    [
                        sum((self.Kx_fft * m_fft), 1),
                        sum((self.Ky_fft * m_fft), 1),
                        sum((self.Kz_fft * m_fft), 1),
                    ],
                    1,
                ),
                dim=(2, 3),
        ...
        self.B = (
            B_demag[0, :, nx - 1 : 2 * nx - 1, ny - 1 : 2 * ny - 1, 0]
            * self.Ms_CoPt
            * mu0
        )
        self.B[1,] += self.B0
