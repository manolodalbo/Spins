...
def turn_into_wave(inputs,embedding_matrix):
    inputs = embedding_matrix[inputs.long()] # should return batch_size x words x embed_size
...
def fm_simpler(inputs: torch.tensor,Fi:float = 0.5e9,Ff:float = 10e9):
    ...
    dt = 20e-12
    t = torch.arange(0,points_per_input * dt,dt,device=inputs.device).unsqueeze(0).unsqueeze(0).unsqueeze(0)
...
def fm(inputs: torch.tensor, Fi: float =0.5e9, Ff: float=10e9) -> torch.tensor:
    ...
    """
    inputs = scaled_sigmoid(inputs,0.3)
    points_per_input = 600//inputs.shape(1)
    ...
    for i in range(inputs.shape(0)):
        ...
        for j, pixel_intensity in enumerate(inputs[i]):
            ...
            if j > 0:
                ...
                modulated_wave[i, points_per_input * j : points_per_input * (j + 1)] =torch.sin(2 * torch.pi * frequency * t[1 : points_per_input + 1] + phase)
                if torch.cos(2 * torch.pi * frequency * t[points_per_input] + phase) > 0:
                    pos_deriv = True
                else:
                    pos_deriv = False
