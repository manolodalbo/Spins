...
class MyTrigram(nn.Module):
    def __init__(self, vocab_size, batch_size, embed_size=80, Bt=0.01):
        ...
        self.embedding_matrix = nn.Parameter(
            torch.normal(torch.zeros(self.vocab_size, self.embed_size), std=5)
        )
    ...
    def forward(self, inputs):
        ...
        film_output = (
            (film_output.sum(dim=-1) - 7.8e6) * 5 / 2.2e6
        )  # need to do some sort of normalization here to get it in the general range [-5,5]. Need to do analysis on what typical std and mean are to normalize
        print(
            f"output mean: {film_output.mean()}  output std: {film_output.std()}"
        )  # should be around 0 and 5
    ...
    def euclidean_distance(self, outputs):
        ...
        """
        distances = outputs.unsqueeze(1) - self.embedding_matrix.unsqueeze(
            0
...
def main():
    ...
    for epoch in range(epochs):
        for i in range(0, len(X0), batch_size):
            ...
            spintorch.plot.plot_loss(loss_iter, plotdir, "loss first trigram model")
